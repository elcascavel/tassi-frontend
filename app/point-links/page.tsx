...
export default function Page() {
  ...
  const getMapName = (id: number) => maps.find((m) => m.id === id)?.name || `map: ${id}`

  const getPointDisplay = (id: number) => {
    const p = points.find((pt) => pt.id === id)
    return p ? `Point #${p.id} (x: ${p.x}, y: ${p.y}, ${getMapName(p.map_id)})` : `#${id}`
  }
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>Select two points to link.</DialogDescription>
              </DialogHeader>
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="point1" className="text-right">From Point</Label>
                  <select id="point1" value={point1} onChange={(e) => setPoint1(e.target.value)} className="col-span-3 border rounded px-3 py-2">
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="point2" className="text-right">To Point</Label>
                  <select id="point2" value={point2} onChange={(e) => setPoint2(e.target.value)} className="col-span-3 border rounded px-3 py-2">
                    <option value="" disabled>Select point</option>
                    {points.map((p) => (
                      <option key={p.id} value={p.id}>
                        #{p.id} (x: {p.x}, y: {p.y}, {getMapName(p.map_id)})
