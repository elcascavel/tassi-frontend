...
export default function Page() {
  ...
  const savePoint = async () => {
    ...
    let res: Response
    let data: { data: { point: Point } }

    if (editing) {
      res = await fetch(`/api/maps/points/${editing.id}`, {
        method: "PUT",
      ...
      data = await res.json()
      if (res.ok) {
        setPoints(prev => prev.map(p => p.id === editing.id ? data.data.point : p))
      }
    } else {
  ...
  const deletePoint = async (id: number) => {
    try {
      const res = await fetch(`/api/maps/points/delete/${id}`, { method: "DELETE" })
      if (res.ok) {
  ...
  const columns: ColumnDef<Point>[] = [
    ...
    { accessorKey: "x", header: "X", cell: ({ row }) => row.original.x.toFixed(2) },
    { accessorKey: "y", header: "Y", cell: ({ row }) => row.original.y.toFixed(2) },
    {
      accessorKey: "map_id",
      header: "Map",
      cell: ({ row }) => {
        ...
        <div className="text-right">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              <DropdownMenuItem className="text-red-500" onClick={() => setToDelete(row.original)}>
                Delete
              </DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
      ...
      <SidebarInset>
        ...
        <div className="p-4 space-y-4">
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{editing ? "Edit Point" : "Create Point"}</DialogTitle>
                <DialogDescription>
                  {editing
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input id="x" type="number" step="any" value={x} onChange={e => setX(e.target.value)} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="y" className="text-right">Y</Label>
                  <Input id="y" type="number" step="any" value={y} onChange={e => setY(e.target.value)} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label htmlFor="map_id" className="text-right">Map</Label>
                  <select
                    id="map_id"
                    value={mapId}
                ...
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <select id="enabled" value={enabled ? "true" : "false"} onChange={e => setEnabled(e.target.value === "true")} className="col-span-3 border rounded px-3 py-2">
              ...
              <DialogFooter>
                ...
                <Button onClick={savePoint}>{editing ? "Save" : "Create"}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
            ...
            <AlertDialog open onOpenChange={o => !o && setToDelete(null)}>
              <AlertDialogContent>
                ...
                <AlertDialogFooter>
                  ...
                  <AlertDialogAction className="text-red-600" onClick={() => { deletePoint(toDelete.id); setToDelete(null); }}>
                    Delete
                  </AlertDialogAction>
                </AlertDialogFooter>
              </AlertDialogContent>
