...
export default function Page() {
  ...
  const save = async () => {
    ...
    if (editing) {
      ...
      const json = await res.json()
      if (res.ok) setCategories((p) => p.map((c) => (c.id === editing.id ? json.data : c)))
    } else {
  ...
  const del = async (id: number) => {
    const res = await fetch(`/api/support/categories/delete/${id}`, { method: "DELETE" })
        ...
        <AlertDialog open onOpenChange={(o) => !o && setCatToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Delete <strong>{catToDelete.name}</strong>? This action cannot be undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
