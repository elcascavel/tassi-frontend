...
export default function Page() {
  ...
  const save = async () => {
    ...
    if (editing) {
      ...
      const json = await res.json();
      if (res.ok)
        setTypes((p) => p.map((t) => (t.id === editing.id ? json.data : t)));
    } else {
  ...
  const del = async (id: number) => {
    const res = await fetch(`/api/beacons/types/delete/${id}`, {
      method: 'DELETE',
    });
        ...
        <AlertDialog open onOpenChange={(o) => !o && setToDelete(null)}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Delete <strong>{toDelete.name}</strong>? This action cannot be
                undone.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
